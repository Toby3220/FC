...
class CubicSpline1D:
    def __init__(self,X_knots:np.ndarray) -> None:
        self.pointer_lim = (X_knots).shape[0]-2
        self.known_x_mappings = dict()
        self.X_knots = X_knots
        pass

    def _get_lp(self,xpoint:float)->int:
        #Priveate, method to get "left pointer", 
        # a pointer that points to the relavent segment of cubic spline 
        if xpoint in self.known_x_mappings:
            lp = self.known_x_mappings[xpoint]
        else:
            # binary search algo
            rp = self.pointer_lim
            lp = self._bucket_binary_search(xpoint,0,rp)
            self.known_x_mappings[xpoint] = lp
        return lp
    ...
    def _get_vec_of_lps(self,times,sorted:bool) -> np.ndarray:
        ...
        while vp <= vmax and lp <= lmax:
            if (self.X_knots[lp]) > (times[vp]):
                if lp == 0:
                    vp+=1
                    continue
                else:
                    self.known_x_mappings[times[vp]]=lp-1
                    out[vp] = lp-1
                    vp+=1
                    continue
            elif (self.X_knots[lp]) <= times[vp]:
                    lp+=1
                    continue
    ...
    def gen_1d_spline_func(self,Y:list)->dict: 
    ...
    def eval_point(self,V:dict,xpoint:float)->float:
        # evaluate single point implied volaility
        ## INPUT
        # xpoint:       float = point on x axis for function to be evaluated
        ## OUTPUT
        # out :         float = evaluated function value (y) at xpoint

        lp = self._get_lp(xpoint)
    ...
    def eval_vec(self,V:dict,X_points:np.ndarray,sorted:bool = False):
        n = X_points.shape[0]
        out = np.zeros(n)
        if sorted:
            self._get_vec_of_lps(X_points,True)
        
        for xpoint,pos in zip(X_points,range(n)):
            lp = self._get_lp(xpoint)
            out[pos] = self.eval_point(V,xpoint)

        return out
    

class CubicSplineSurface(CubicSpline1D):
    def __init__(self,H,params) -> None:
        ...
        super().__init__(self.maturities)
        pass
    ...
    def gen_1d_spline_func(self,price:float)->dict: 
        ...
        X_data = gen_maturity_vol_func_dataset(price,self.X_knots,self.params)
        vol_est = (X_data*self.H).sum(1)
        
        return super().gen_1d_spline_func(vol_est.tolist())
    ...
    def eval_imp_vol(self,price:float,maturity:float)->float:
        ...
        V = self._get_V(price)
        vol = self.eval_point(V,maturity)
    ...
    def eval_local_vol(self,price:float,time:float, from_imp_vol_surface:bool = False):
        ...
        if not from_imp_vol_surface and local_vol <-0.02: 
            print("")
        return local_vol          
